...
import pl.gpiwosz.accountancy.repository.ContentRepository;
...
/**
 * REST controller for managing {@link pl.gpiwosz.accountancy.domain.Content}.
 */
@RestController
@RequestMapping("/api")
public class ContentResource {
    ...
    public List<Content> getAllContents(@RequestParam(required = false) String filter) {
        if ("invoice-is-null".equals(filter)) {
            log.debug("REST request to get all Contents where invoice is null");
            return StreamSupport
                .stream(contentRepository.findAll().spliterator(), false)
                .filter(content -> content.getInvoice() == null)
