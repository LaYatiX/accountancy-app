            ...
            <tbody>
              {incomeList.map((income, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={income.date} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
                  <td>{income.monthSumUp ? <Link to={`month-sum-up/${income.monthSumUp.id}`}>{income.monthSumUp.id}</Link> : ''}</td>
