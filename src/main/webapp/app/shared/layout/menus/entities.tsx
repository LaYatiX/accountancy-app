import React from 'react';
import MenuItem from 'app/shared/layout/menus/menu-item';
import { DropdownItem } from 'reactstrap';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { Translate, translate } from 'react-jhipster';
import { NavLink as Link } from 'react-router-dom';
import { NavDropdown } from './menu-components';

export const EntitiesMenu = props => (
  // tslint:disable-next-line:jsx-self-close
  <NavDropdown icon="th-list" name="Entities" id="entity-menu">
    <MenuItem icon="asterisk" to="/entity/invoice">
      Faktury
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/content">
      Content
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/month-sum-up">
      Month Sum Up
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/expense">
      Expense
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/income">
      Income
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/entry">
      Entry
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/company">
      Company
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/contractor">
      Contractor
    </MenuItem>
    <MenuItem icon="asterisk" to="/entity/product">
      Product
    </MenuItem>
    {/* jhipster-needle-add-entity-to-menu - JHipster will add entities to the menu here */}
  </NavDropdown>
);
